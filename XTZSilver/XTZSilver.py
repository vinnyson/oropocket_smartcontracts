...
TZIP16_Metadata_Base = {
    ...
    "description"   : "Silver tokenized on Tezos",
    "authors"       : [
        "Oropocket <https://twitter.com/oropocket>"
    ],
    "homepage"      : "https://oropocket.com/",
    "interfaces"    : [
        "TZIP-007-2021-04-17",
        "TZIP-016-2021-04-17"
    ],
...
if "templates" not in __name__:
    ...
    def test():
            ...
            sp.some(
                sp.record(
                    ...
                    token_info = sp.map({
                        ...
                        "name"        : sp.utils.bytes_of_string("Oropocket Silver"),
                        "symbol"      : sp.utils.bytes_of_string("XTZSilver"),
                        "thumbnailUri"        : sp.utils.bytes_of_string('https://oropocket.com/unifarm/assets/icons/oro-silver.png')
                    })
                )
            )
        )
        ...
        scenario.verify(
            c1.data.metadata[""] == sp.utils.bytes_of_string("ipfs://QmYuu9tMdjty8FWie37yFrncA7oZC3MQwVhcddZpipvr4a")
        )
        ...
        c1.mint(address = alice.address, value = 12000000).run(sender = admin)
        c1.mint(address = alice.address, value = 3000000).run(sender = admin)
        c1.mint(address = alice.address, value = 3000000).run(sender = admin)
        scenario.h2("Alice transfers to Bob")
        c1.transfer(from_ = alice.address, to_ = bob.address, value = 4000000).run(sender = alice)
        scenario.verify(c1.data.balances[alice.address].balance == 14000000)
        scenario.h2("Bob tries to transfer from Alice but he doesn't have her approval")
        c1.transfer(from_ = alice.address, to_ = bob.address, value = 4000000).run(sender = bob, valid = False)
        scenario.h2("Alice approves Bob and Bob transfers")
        c1.approve(spender = bob.address, value = 5000000).run(sender = alice)
        c1.transfer(from_ = alice.address, to_ = bob.address, value = 4000000).run(sender = bob)
        scenario.h2("Bob tries to over-transfer from Alice")
        c1.transfer(from_ = alice.address, to_ = bob.address, value = 4000000).run(sender = bob, valid = False)
        scenario.h2("Admin burns Bob token")
        c1.burn(address = bob.address, value = 1000000).run(sender = admin)
        scenario.verify(c1.data.balances[alice.address].balance == 10000000)
        scenario.h2("Alice tries to burn Bob token")
        c1.burn(address = bob.address, value = 1000000).run(sender = alice, valid = False)
        ...
        c1.transfer(from_ = alice.address, to_ = bob.address, value = 1000000).run(sender = admin)
        scenario.h2("Admin unpauses the contract and transferts are allowed")
        c1.setPause(False).run(sender = admin)
        scenario.verify(c1.data.balances[alice.address].balance == 9000000)
        c1.transfer(from_ = alice.address, to_ = bob.address, value = 1000000).run(sender = alice)

        scenario.verify(c1.data.totalSupply == 17000000)
        scenario.verify(c1.data.balances[alice.address].balance == 8000000)
        scenario.verify(c1.data.balances[bob.address].balance == 9000000)
        ...
        scenario.verify_equal(view_balance.data.last, sp.some(8000000))
